...
__device__ __forceinline__ float3 atmosphere::getSunAndSkyIrradiance(
	...
	const cudaTextureObject_t transmittance_texture,
	const cudaTextureObject_t irradiance_texture,
	const float3 float3, const float3 normal, const float3 sun_direction,
	float3 &sky_irradiance) {

	float r = float(float3);
	float mu_s = dot(float3, sun_direction) / r;

	sky_irradiance = getIrradiance(atmosphere, irradiance_texture, r, mu_s) * (1.0f + dot(normal, float3) / r) * 0.5f;
...
__device__ __forceinline__ float atmosphere::ClampCosine(float mu) {
	return clamp(mu, float(-1.0), float(1.0));
}

__device__ __forceinline__ float atmosphere::ClampDistance(float d) {
	return max(d, 0.0 * m);
}

__device__ __forceinline__ float atmosphere::ClampRadius(const AtmosphereParameters atmosphere, float r) {
	return clamp(r, atmosphere.bottom_radius, atmosphere.top_radius);
}

__device__ __forceinline__ float atmosphere::SafeSqrt(float a) {
	return sqrt(max(a, 0.0 * m2));
}

__device__ __forceinline__ float atmosphere::DistanceToTopAtmosphereBoundary( const AtmosphereParameters atmosphere,
	float r, float mu) {
...
__device__ __forceinline__ float atmosphere::DistanceToBottomAtmosphereBoundary(const AtmosphereParameters atmosphere,
	float r, float mu) {
...
__device__ __forceinline__ bool atmosphere::RayIntersectsGround(const AtmosphereParameters atmosphere,
	float r, float mu) {
	assert(r >= atmosphere.bottom_radius);
	assert(mu >= -1.0 && mu <= 1.0);
	return mu < 0.0 && r * r * (mu * mu - 1.0) +
...
__device__ __forceinline__ float atmosphere::GetLayerDensity(const DensityProfileLayer layer, float altitude) {
...
__device__ __forceinline__ float atmosphere::GetProfileDensity(const DensityProfile profile, float altitude) {
	return altitude < profile.layers[0].width ?
		GetLayerDensity(profile.layers[0], altitude) :
		GetLayerDensity(profile.layers[1], altitude);
}

__device__ __forceinline__ float atmosphere::ComputeOpticalLengthToTopAtmosphereBoundary(
...
__device__ __forceinline__ float3 atmosphere::ComputeTransmittanceToTopAtmosphereBoundary(
...
__device__ __forceinline__ float2 atmosphere::GetTransmittanceTextureUvFromRMu(const AtmosphereParameters atmosphere,
	float r, float mu) {
	assert(r >= atmosphere.bottom_radius && r <= atmosphere.top_radius);
	assert(mu >= -1.0 && mu <= 1.0);
...
__device__ __forceinline__ void atmosphere::GetRMuFromTransmittanceTextureUv(const AtmosphereParameters atmosphere, float2 uv, float &r, float &mu) {
...
__device__ __forceinline__ float3 atmosphere::ComputeTransmittanceToTopAtmosphereBoundaryTexture(
...
__device__ __forceinline__ float3 atmosphere::GetTransmittanceToTopAtmosphereBoundary(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	float r, float mu) {
	assert(r >= atmosphere.bottom_radius && r <= atmosphere.top_radius);
...
__device__ __forceinline__ float3 atmosphere::GetTransmittance(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	float r, float mu, float d, bool ray_r_mu_intersects_ground) {
	assert(r >= atmosphere.bottom_radius && r <= atmosphere.top_radius);
...
__device__ __forceinline__ float3 atmosphere::GetTransmittanceToSun(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	float r, float mu_s) {
	float sin_theta_h = atmosphere.bottom_radius / r;
...
__device__ __forceinline__ float atmosphere::DistanceToNearestAtmosphereBoundary(const AtmosphereParameters atmosphere,
...
__device__ __forceinline__ float atmosphere::RayleighPhaseFunction(float nu) {
	float k = 3.0 / (16.0 * PI * sr);
	return k * (1.0 + nu * nu);
}

__device__ __forceinline__ float atmosphere::MiePhaseFunction(float g, float nu) {
	float k = 3.0 / (8.0 * PI * sr) * (1.0 - g * g) / (2.0 + g * g);
	return k * (1.0 + nu * nu) / pow(1.0 + g * g - 2.0 * g * nu, 1.5);
}

__device__ __forceinline__ float4 atmosphere::GetScatteringTextureUvwzFromRMuMuSNu(const AtmosphereParameters atmosphere,
	float r, float mu, float mu_s, float nu,
	bool ray_r_mu_intersects_ground) {
	assert(r >= atmosphere.bottom_radius && r <= atmosphere.top_radius);
	assert(mu >= -1.0 && mu <= 1.0);
...
__device__ __forceinline__ void atmosphere::GetRMuMuSNuFromScatteringTextureUvwz(const AtmosphereParameters atmosphere,
...
__device__ __forceinline__ void atmosphere::ComputeSingleScatteringTexture(const AtmosphereParameters atmosphere,
...
__device__ __forceinline__ float3 atmosphere::GetScattering(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t scattering_texture,
	float r, float mu, float mu_s, float nu,
	bool ray_r_mu_intersects_ground) {
...
__device__ __forceinline__ float3 atmosphere::ComputeScatteringDensity(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	const cudaTextureObject_t single_rayleigh_scattering_texture,
	const cudaTextureObject_t single_mie_scattering_texture,
...
__device__ __forceinline__ float3 atmosphere::ComputeMultipleScattering(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
...
__device__ __forceinline__ float3 atmosphere::ComputeScatteringDensityTexture(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	const cudaTextureObject_t single_rayleigh_scattering_texture,
	const cudaTextureObject_t single_mie_scattering_texture,
...
__device__ __forceinline__ float3 atmosphere::ComputeMultipleScatteringTexture(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	const cudaTextureObject_t scattering_density_texture,
...
__device__ __forceinline__ float3 atmosphere::ComputeDirectIrradiance(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	float r, float mu_s) {
	assert(r >= atmosphere.bottom_radius && r <= atmosphere.top_radius);
...
__device__ __forceinline__ float3 atmosphere::ComputeIndirectIrradiance(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t single_rayleigh_scattering_texture,
	const cudaTextureObject_t single_mie_scattering_texture,
	const cudaTextureObject_t multiple_scattering_texture,
...
__device__ __forceinline__ float2 atmosphere::GetIrradianceTextureUvFromRMuS(const AtmosphereParameters atmosphere,
	float r, float mu_s) {
	assert(r >= atmosphere.bottom_radius && r <= atmosphere.top_radius);
	assert(mu_s >= -1.0 && mu_s <= 1.0);
	float x_r = (r - atmosphere.bottom_radius) /
...
__device__ __forceinline__ void atmosphere::GetRMuSFromIrradianceTextureUv(const AtmosphereParameters atmosphere,
...
const float2 IRRADIANCE_TEXTURE_SIZE = make_float2(IRRADIANCE_TEXTURE_WIDTH, IRRADIANCE_TEXTURE_HEIGHT);

__device__ __forceinline__ float3 atmosphere::ComputeDirectIrradianceTexture(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	float2 frag_coord) {
	float r;
...
__device__ __forceinline__ float3 atmosphere::ComputeIndirectIrradianceTexture(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t single_rayleigh_scattering_texture,
	const cudaTextureObject_t single_mie_scattering_texture,
	const cudaTextureObject_t multiple_scattering_texture,
...
__device__ __forceinline__ float3 atmosphere::GetIrradiance(
	...
	float r, float mu_s) {
	float2 uv = GetIrradianceTextureUvFromRMuS(atmosphere, r, mu_s);
	return float3(texture(irradiance_texture, uv));
}

#ifdef COMBINED_SCATTERING_TEXTURES
__device__ __forceinline__ float3 atmosphere::GetExtrapolatedSingleMieScattering(
	const AtmosphereParameters atmosphere, IN(float4) scattering) {
	if (scattering.r == 0.0) {
...
__device__ __forceinline__ float3 atmosphere::GetCombinedScattering(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t scattering_texture,
	const cudaTextureObject_t single_mie_scattering_texture,
	float r, float mu, float mu_s, float nu,
...
__device__ __forceinline__ float3 atmosphere::GetSkyRadiance(
	const AtmosphereParameters atmosphere,
	const cudaTextureObject_t transmittance_texture,
	const cudaTextureObject_t scattering_texture,
	const cudaTextureObject_t single_mie_scattering_texture,
